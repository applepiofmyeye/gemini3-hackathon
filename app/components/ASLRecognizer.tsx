  ...
  const connect = useCallback(() => {
    ...
    ws.onopen = () => {
      ...
      const setupMsg = {
        setup: {
          ...
          systemInstruction: {
             parts: [{ text: "You are an expert ASL (American Sign Language) translator. Your task is to transcribe the hand gestures in the video stream into text. Output only the translation. If it is a letter, output the letter. If it is a word, output the word. Do not hallucinate. If no clear gesture is seen, output nothing or wait." }]
          }
        }
      };
    ...
    ws.onmessage = (event) => {
        ...
        const parseMessage = async (data: Blob | string) => {
            let textData = typeof data === 'string' ? data : await data.text();
            try {
  ...
  return (
    <div className="flex flex-col items-center gap-6 p-6 max-w-2xl mx-auto w-full">
      ...
      <div className="w-full p-6 bg-gray-900/50 rounded-2xl backdrop-blur-md border border-white/10 min-h-[150px]">
        <h3 className="text-xs font-mono text-gray-400 mb-2 uppercase tracking-wider">Transcription</h3>
